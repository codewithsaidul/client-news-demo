...
const FeatureNews = ({ businessNews }) => {
  return (
    <div>
      <figure className="relative">
        <Image
          src={businessNews?.thumbnail}
        ...
        <Button className="text-xl font-title py-5 px-7 absolute -bottom-4 left-1/2 transform -translate-x-1/2">
          {businessNews.category === "Finance"
            ? "Economy and Finance"
            : businessNews.category}
        </Button>
      </figure>
      ...
      <div className="text-center mt-8">
        ...
        <p className="text-lg mt-5 text-news-text">
          <span className="font-bold">{businessNews.author.name}</span>

          <span className="mx-2">|</span>

          <span>{dateFormater(businessNews.createdAt)}</span>
        </p>
      </div>
    </div>
  );
